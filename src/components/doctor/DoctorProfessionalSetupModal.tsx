...
type DoctorProfessionalSetupModalProps = {
  open: boolean;
  onComplete: () => void;
};

export default function DoctorProfessionalSetupModal({
  open,
  onComplete,
}: DoctorProfessionalSetupModalProps) {
  if (!open) return null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="doctor-professional-setup-title"
      ...
      >
        <div className="border-b border-teal-50 px-6 py-5">
          <h2
            id="doctor-professional-setup-title"
            className="text-lg font-semibold text-teal-800"
          >
            Complete your professional profile
          </h2>
          <p className="mt-1 text-sm text-zinc-500">
            Add your license, specialization, and availability before accepting
            appointments.
          </p>
        </div>

        <div className="overflow-y-auto scrollbar-hide px-6 py-5">
          <DoctorProfileForm mandatory onComplete={onComplete} />
        </div>
      </div>
    </div>
  );
