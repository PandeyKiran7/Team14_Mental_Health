...
export default function DoctorRecommendationPage() {
  ...
  const loadBookings = useCallback(async () => {
    ...
    if (!token) {
      setError("Please log in to manage recommendations.");
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-3 rounded-xl border border-teal-100 bg-white p-6 shadow-sm">
        ...
        <div>
          <h1 className="text-2xl font-bold text-zinc-800" id="recommendations-title">Recommendations</h1>
          <p className="text-sm text-zinc-500">
            Create, edit, and download dietary and lifestyle recommendations for your confirmed and completed appointments.
          </p>
        </div>
      </div>
      ...
      {!loading && !error && bookings.length === 0 && (
        <div className="rounded-xl border border-dashed border-zinc-200 bg-white py-12 text-center">
          ...
          <p className="mt-1 text-sm text-zinc-400">
            Recommendations can only be created for approved appointments.
          </p>
        </div>
      )}
